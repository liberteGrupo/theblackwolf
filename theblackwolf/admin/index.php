...
echo $dataSeparada[0].'/'; // 2013
echo $dataSeparada[1].'/'; // 03
echo $dataSeparada[2]; // 21 
$dataSeparada  = $hoje;
$sql ="SELECT * FROM itens_pedidos RIGHT OUTER JOIN pedido ON pedido.ID_PEDIDO =  itens_pedidos.ID_PEDIDO WHERE DATA_ITEM_PEDIDO = '$hoje' ";
$result = mysql_query($sql);
...
echo '<tr>';
echo '<td> <label>Cód Pedido</label></td>';
...
while($pedido = mysql_fetch_array($result)){
    ...
    $timestamp = strtotime($date); // Gera o timestamp de $data_mysql
    $data      = date('d/m/Y', $timestamp); // Resultado: 12/03/    
    echo '<td>'.$data.'</td>';
...
echo "<div class='totalCompras'><h2>Total em compras efetuadados:<h2><b class='numero_pedido_texto' >R$".$totalCompras."</b></div>";
}
?>
        <br><div  class='cadastraMeta' >
