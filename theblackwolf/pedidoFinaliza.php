    ...
    <td colspan="2"></td>
	 ...
	 $data_hora = date('Y-m-d H:i:s');
         ...
         if(isset($_POST['enviar'])){
             if(isset($_POST['frete']) && $_POST['frete']  === '1'){
            $frete = '1';
              }elseif(isset($_POST['frete']) && $_POST['frete'] === '2'){
            $frete = '2';     
             }
            if($cod_usuario === ''   or empty($_SESSION['nomeUsuario']))
            {   
                echo '<script> alert("Você precisa estár logado para comprar!,faça o login agora");
  ...
  <label>Endereco de onde será entregue o produto</label>
     ...
     echo '<b>Numero :</b>'  .$usuarios['NUMERO'].'<br>'; 
      ...
      <label>Escolha a forma de envio :</label><br>
          <input type="radio" name="frete" value="1" checked="ta ai">Encomenda normal(PAC)<br>
      <input type="radio" name="frete" value="2">SEDEX<br>
   </div>
</form>
